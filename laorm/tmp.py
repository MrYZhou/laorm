...
class LaModel(metaclass=ABCMeta):
    ...
    async def dynamic(cls: type[T], dynamicSql: str, params: str | list = None):
        ...
        try:
            ...
            res = await cls.exec(params=params, fetch_one=False)
            if not cls.cacheSql.get(dynamicSql):
                cls.cacheSql[dynamicSql] = cls.finalize()
...
def sql(func):
    def wrapper(cls, *args, **kwargs):
        ...
        if not cls.cacheSql.get(method_cache_name):
            cls.cacheSql[method_cache_name] = cls.finalize()
        return res
